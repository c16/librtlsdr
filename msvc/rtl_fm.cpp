...
static void rtlsdr_callback(unsigned char *buf, uint32_t len, void* ctx)
{
	...
	d->lp_len = len;
	d->rw.unlock();
	notify(d->ready, d->ready_m);
}

static void *dongle_thread_fn(dongle_state*arg)
{
	struct dongle_state *s = arg;
	rtlsdr_read_async(s->dev, rtlsdr_callback, (void *)s, 0, s->buf_len);
	return 0;
}

static void *demod_thread_fn(demod_state*arg)
{
	...
	while (!do_exit) {
		...
		if (d->squelch_level && d->squelch_hits > d->conseq_squelch) {
			...
			notify(controller.hop, controller.hop_m);
			continue;
		}
		o->rw.lock();
		...
		notify(o->ready, o->ready_m);
	}
	
	return 0;
}

static void *output_thread_fn(output_state*arg)
{
	struct output_state *s = arg;
	while (!do_exit) {
		// use timedwait and pad out under runs
		
		std::unique_lock<std::mutex> lck(s->ready_m);
		s->ready.wait(lck);
...
static void *controller_thread_fn(controller_state*arg)
{
	...
	while (!do_exit) {
		std::unique_lock<std::mutex> lck(s->hop_m);
		s->hop.wait(lck);
		if (s->freq_len <= 1) {
			continue;}
		/* hacky hopping */
...
int main(int argc, char **argv)
{
	...
	if (!dev_given) {
		char zero[2] = "0";
		dongle.dev_index = verbose_device_search(zero);
	}
	...
	controller.thread = std::thread(controller_thread_fn, &controller);
	
	usleep(100000);
	output.thread = std::thread(output_thread_fn, &output);
	demod.thread = std::thread(demod_thread_fn, &demod);
	dongle.thread = std::thread(dongle_thread_fn, &dongle);
	...
	notify(demod.ready, demod.ready_m);
	demod.thread.join();
	notify(output.ready, output.ready_m);
	output.thread.join();
	notify(controller.hop, controller.hop_m);
